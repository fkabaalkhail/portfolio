...
const chips = ["✦ 4 luxury categories", "★ Michelin-tagged venues", "⚡ 3-step booking", "🎟 Apple Wallet pass"];
...
export default function MrasemShowcase() {
  ...
  return (
    <div className="relative overflow-clip rounded-[2rem] bg-espresso text-cream md:rounded-[3rem]">
      {/* warm glow */}
      <div aria-hidden className="pointer-events-none absolute -right-40 -top-40 h-[40rem] w-[40rem] rounded-full bg-gold/20 blur-[140px]" />
      <div aria-hidden className="pointer-events-none absolute -bottom-40 -left-40 h-[30rem] w-[30rem] rounded-full bg-ember/15 blur-[140px]" />
      ...
      <div className="relative px-6 pt-14 md:px-14 md:pt-20">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <span className="rounded-full border border-gold/30 px-3 py-1 font-mono text-[11px] uppercase tracking-[0.18em] text-gold">
            01 · iOS + Admin + API
          </span>
        </div>
        <h3 className="mt-8 max-w-4xl font-display text-[clamp(2.4rem,6vw,5rem)] font-bold leading-[0.95] tracking-[-0.04em]">
          ...
          <SplitWords text="exclusively Saudi." delay={0.15} wordClassName="font-serif font-normal italic text-gold" />
        </h3>
        <Reveal delay={0.2}>
          <p className="mt-6 max-w-2xl text-lg leading-relaxed text-cream/65">{project.description}</p>
        </Reveal>
      </div>
          ...
          {features.map((f, i) => (
            ...
            >
              ...
              >
                ...
                <h4 className="mt-3 font-display text-3xl font-bold tracking-tight md:text-4xl">{f.title}</h4>
                <p className="mt-4 text-lg leading-relaxed text-cream/60">{f.body}</p>
        ...
        <div className="order-1 hidden lg:order-2 lg:block">
          <div className="sticky top-0 flex h-screen items-center justify-center pb-10">
            <div className="relative w-[300px] xl:w-[330px]">
              ...
              {/* floating callout chip (Monologue-style) */}
              <AnimatePresence mode="wait">
                <motion.span
                  key={chips[active]}
                  initial={{ opacity: 0, scale: 0.7, rotate: -12, x: 20 }}
                  animate={{ opacity: 1, scale: 1, rotate: -5, x: 0 }}
                  exit={{ opacity: 0, scale: 0.8, rotate: 4 }}
                  transition={{ type: "spring", stiffness: 260, damping: 18 }}
                  className="absolute -right-24 top-[22%] z-20 whitespace-nowrap rounded-2xl bg-gold px-4 py-2.5 text-sm font-semibold text-espresso shadow-2xl"
                >
                  {chips[active]}
                </motion.span>
              </AnimatePresence>
            ...
            <div className="absolute bottom-10 left-1/2 flex -translate-x-1/2 gap-1 rounded-full border border-cream/10 bg-espresso/80 p-1 backdrop-blur-xl">
              {features.map((f, i) => (
                <button
                  key={f.title}
                  onClick={() => stepRefs.current[i]?.scrollIntoView({ behavior: "smooth", block: "center" })}
                  className={`relative rounded-full px-4 py-2 text-sm font-medium transition-colors ${active === i ? "text-espresso" : "text-cream/60 hover:text-cream"}`}
                >
                  {active === i && (
                    <motion.span
                      layoutId="mrasem-tab"
      ...
      <div className="relative mt-10 md:mt-16">
        <div className="flex flex-wrap items-end justify-between gap-4 px-6 md:px-14">
          <Reveal>
            <p className="font-mono text-xs uppercase tracking-[0.2em] text-gold">Design system</p>
            <h4 className="mt-2 font-display text-3xl font-bold tracking-tight md:text-4xl">
              Every screen, bilingual EN / AR
            </h4>
          </Reveal>
          <p className="font-mono text-xs text-cream/40">← drag to explore →</p>
        </div>
        <div ref={galleryRef} className="mt-8 overflow-hidden px-6 pb-4 md:px-14">
          ...
          >
            {gallery.map((g, i) => (
              ...
              >
                <div className="pointer-events-none overflow-hidden rounded-[1.6rem] border border-cream/10 shadow-2xl">
                  <Image
                    src={g.src}
                ...
                <figcaption className="mt-3 text-sm text-cream/55">{g.label}</figcaption>
              </motion.figure>
            ))}
          </motion.div>
        </div>
      ...
      <div ref={dashRef} className="relative mt-16 px-6 [perspective:1600px] md:px-14">
        <motion.div
          ...
          className="mx-auto max-w-5xl origin-bottom overflow-hidden rounded-2xl border border-cream/10 bg-[#1c130e] shadow-[0_50px_120px_-30px_rgba(0,0,0,0.7)]"
        >
          <div className="flex items-center gap-2 border-b border-cream/10 px-4 py-3">
            <span className="h-3 w-3 rounded-full bg-[#ff5f57]" />
            <span className="h-3 w-3 rounded-full bg-[#febc2e]" />
            <span className="h-3 w-3 rounded-full bg-[#28c840]" />
            <span className="mx-auto rounded-md bg-cream/5 px-6 py-1 font-mono text-xs text-cream/40">
      ...
      <div className="relative flex flex-col gap-6 px-6 py-12 md:flex-row md:items-center md:justify-between md:px-14 md:py-16">
        <div className="flex flex-wrap gap-2">
          {project.tech.map((t) => (
            <span key={t} className="rounded-full border border-cream/15 px-3 py-1.5 text-sm text-cream/70">
              {t}
            </span>
          ))}
        </div>
        <div className="flex flex-wrap gap-3">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full border border-gold/40 px-5 py-3 text-sm font-semibold text-gold transition-colors hover:bg-gold/10"
          >
          ...
          <a
            ...
            className="group inline-flex items-center gap-2 rounded-full bg-gold px-5 py-3 text-sm font-semibold text-espresso transition-transform hover:scale-[1.03]"
          >
            View source
